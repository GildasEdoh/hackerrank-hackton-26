...
class CashFlowSimulator:
    ...
    def get_category_recurrence(self, user_id, request_date_str):
    ...
    def forecast_user_cashflow(self, user_id, request_date_str, spending_changes=None):
        ...
        # Base salary only (exclude bonuses, commissions, one-offs per §6.3)
        salary_events = [
        ...
        # B. Monthly recurring categories (rent, utilities, loans, insurance, education, gym, subscriptions)
        monthly_cats = {
        ...
        # Check by description for monthly items
        ...
        # C. Variable spending categories with dynamic interval (< 25 days)
...
if __name__ == "__main__":
    ...
    for s in data["sample_requests"][:10]:
        changes = []
        if s["spending_changes_needed"] != "none":
            changes = s["spending_changes_needed"].split("|")
        balances, min_proj, min_bal = sim.simulate_balance(s["user_id"], s["request_date"], changes)
        safe_calc = max(0.0, min_proj - min_bal)
        req_amt = float(s["requested_amount"])
        safe_calc = min(req_amt, safe_calc)
        print(f"{s['request_id']} {s['user_id']}: safe_calc={safe_calc:.2f}, actual={s['amount_safe_to_pay']}")
